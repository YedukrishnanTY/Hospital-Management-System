...
?>
	...
	<div class="about" id="department">
		<div class="container">
		<br/> <br/> <br/> <br/> <br/>
			<h3 class="title">Edit Programs</h3>
			<?php
	
				$id=$_GET['un'];
				$logid=$_SESSION['id'];
							$select="select * from programs where pgmid=$id";
							$res=$obj->exe($select);
							$row=mysqli_fetch_array($res);
						?>
						</p>
				<form name="form_rec" method="post" class="login" enctype="multipart/form-data" action="editpgmpro.php">
                
                <table class="table">
                   
                    <input type="hidden" name="pgmid" required="required" value="<?php echo $row[0];?>"  >
						<tr><td><label>Program Name</label></td><td>
                    <input type="text" name="pgmname" required="required" value="<?php echo $row[1];?>"  ></td></tr>
					<tr><td><label>Hospital Name</label></td><td>
                    <input type="text" name="hosname" required="required" value="<?php echo $row[2];?>" readonly=""  ></td></tr>
					<tr><td><label>Program Date</label></td><td>
                    <input type="text" name="pgmdate" required="required" value="<?php echo $row[3];?>"  ></td></tr>
					<tr><td><label>Program Time</label></td><td>
                    <input type="text" name="pgmtime" required="required" value="<?php echo $row[4];?>"  ></td></tr>
					<tr><td><label>Program Venue</label></td><td>
                    <input type="text" name="pgmvenue" required="required" value="<?php echo $row[5];?>"  ></td></tr>
