...
?>
	...
	<div class="about" id="department">
		<div class="container">
			<h3 class="title">Edit Facility</h3>
			<br/> <br/> <br/> <br/> <br/>
			<?php
	
				$id=$_GET['un'];
				$logid=$_SESSION['id'];
							$select="select * from facilities where fid=$id";
							$res=$obj->exe($select);
							$row=mysqli_fetch_array($res);
						?>
						</p>
				<form name="form_rec" method="post" class="login" enctype="multipart/form-data" action="editfacpro.php">
                
                <table class="table">
                   
                    <input type="hidden" name="fid" required="required" value="<?php echo $row[0];?>"  >
						<tr><td><label>Facility Name</label></td><td>
                    <input type="text" name="fname" required="required" value="<?php echo $row[1];?>"  ></td></tr>
