...
?>
	...
	<div class="about" id="booking">
		...
		<br/> <br/> <br/> <br/> <br/>
			...
			<?php
			 $logid=$_SESSION['id'];
			$sqlb="select * from payment where hosname=(select hosname from hospital where logid=$logid) and status='booked'";
			$resb=$obj->exe($sqlb);
		  $nob=mysqli_num_rows($resb);
		  if($nob==0)
		  {
		  ...
		  ?>
						<tr>
							...
							<td><?php echo $rowb[8];?></td>
							<td><a href="delbook.php?un=<?php echo $rowb[0]?>">Delete</a> / <a href="gena.php?un=<?php echo $rowb[0]?>">Generate Admit Card</a></td>
	...
	<!--subscribe-->
	<div style="background-color:#CCCCCC" class="about" id="booking">
		<div class="container">
			<h3 class="title">Debit Card Details</h3>
			
			<?php
			 $logid=$_SESSION['id'];
			$sqld="select * from hospitalac where hosname=(select hosname from hospital where logid=$logid)";
			$resd=$obj->exe($sqld);
		  ...
		  if($nod==0)
		  {
			 ...
			 <div class="col-md-4 w3layouts_more-buttn">
											<a href="adddebit.php">Add DebitCard</a>
										</div>
										<?php
		  }
		  else
